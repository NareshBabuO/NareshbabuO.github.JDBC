...
public class PersonRepositoryImp implements PersonRepository {
	...
	public void display() {
		...
		try {
			...
			while (result.next()) {
				...
				boolean married = result.getBoolean(6);
				boolean work = result.getBoolean(7);
				String company = result.getString(8);
				double salary = result.getDouble(9);
				int age = result.getInt(10);
				int exp = result.getInt(11);
				String location = result.getString(12);
				String city = result.getString(13);
				String state = result.getString(14);
				String country = result.getString(15);
				long adharNo = result.getLong(16);
				long bankNo = result.getLong(17);
	...
	public void displayAllByGenderDescOrderByName(String gender) {
		...
		try {
			...
			Connection connection = DriverManager.getConnection(JDBCContainer.URL.getContainer(),
					JDBCContainer.USERNAME.getContainer(), JDBCContainer.DONTELL.getContainer());
			String mysql = "select * from xworkzbtm.person where gender="+gender+" order by name desc";
			Statement statement = connection.createStatement();
			ResultSet resultSet = statement.executeQuery(mysql);
			while (resultSet.next()) {
